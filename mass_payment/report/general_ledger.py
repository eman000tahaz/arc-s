    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        if init_balance:
            ...
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, NULL AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
                ...
                WHERE l.account_id IN %s""" + partner_condition['sql'] + filters + ' GROUP BY l.account_id')
            params = (tuple(accounts.ids),) + partner_condition['tpl_ids'] + tuple(init_where_params)
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            ...
            WHERE l.account_id IN %s''' + partner_condition['sql'] + filters + ''' GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.ref, l.name, m.name, c.symbol, p.name ORDER BY ''' + sql_sort)
        params = (tuple(accounts.ids),) + partner_condition['tpl_ids'] + tuple(where_params)
        cr.execute(sql, params)
